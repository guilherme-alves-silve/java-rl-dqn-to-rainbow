...
public class PERL2Loss extends Loss {
     ...
     */
    public void normISWeights(NDArray normISWeights) {
        if (normISWeights.getShape().dimension() == 1) {
            normISWeights = normISWeights.reshape(INFER_DIMENSION, 1);
        }
    ...
    /**
     * Just used to transfer again to be used by the gradient collector and the sub manager.
     * @return normISWeights IS weights with shape (batchSize,) or (batchSize, 1)
     */
    public NDArray normISWeights() {
        return normISWeights;
    }
